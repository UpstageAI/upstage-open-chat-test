	...
	>
		<slot>
			...
			<div class="px-3 max-h-64 overflow-y-auto scrollbar-hidden group relative">
				...
				{#each filteredItems
					.sort((a, b) => (b.value.toLowerCase().startsWith('solar') ? 1 : 0) - (a.value
								.toLowerCase()
								.startsWith('solar') ? 1 : 0))
					.filter((item) => !(item.model?.info?.meta?.hidden ?? false)) as item, index}
					<button
											...
											>
												<img
													src={item.model?.info?.meta?.profile_image_url ??
														(item.label?.toLowerCase().includes('solar')
															? '/static/favicon.svg'
															: '/static/favicon.svg')}
													alt="Model"
